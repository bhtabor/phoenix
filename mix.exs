defmodule Phoenix.MixProject do
  ...
  if Mix.env() != :prod do
    for path <- :code.get_path(),
        Regex.match?(~r/phx_new_hotwire-[\w\.\-]+\/ebin$/, List.to_string(path)) do
  ...
  defp deps do
    [
      ...
      {:phx_new_hotwire, path: "./installer", only: [:docs, :test]},
      {:mint, "~> 1.4", only: :test},
  ...
  defp raise_on_archive_build(_) do
    ...
    You probably meant to install "phx_new_hotwire" instead
    """)
  end
